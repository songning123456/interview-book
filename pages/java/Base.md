...
| :----- | :----- | 
| 1. 发生在父子类中；<br>2. 方法名、参数列表必须相同；<br>3. 返回值类型小于等于父类；<br>4. 抛出的异常小于等于父类；访问修饰符大于等于父类；<br>5. 如果父类方法访问修饰符为private，则子类中就不是重写。 | 1. 发生在同一个类中；<br>2. 方法名必须相同；<br>3. 参数类型不同、个数不同、顺序不同；<br>4. 方法返回值和访问修饰符可以不同；<br>5. 发生在编译时。 | 
...
| 绝对抽象的，不可以被实例化。 | 不可以被实例化，但是如果它包含main方法的话是可以被调用的。 |
